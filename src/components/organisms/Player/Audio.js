...
function Audio(){
    ...
    const {setDuration , setTime, setPosition , setPlay} = useControlActions();
    const {play , volume , position ,time} = useControlState()
    const control = useControlState()

    const {itemIndex , playlist , repeat } = usePlayerState()
    ...
    React.useEffect(()=>{
        ...
        ref.current.onended = ()=>{setEnded(true)};
    },[itemIndex])
    ...
    },[control.play, control.volume , loaded])
