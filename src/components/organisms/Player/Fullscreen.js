...
function Fullscreen(){
    const {fullscreen , playlist, itemIndex} = usePlayerState()
    const {toggleFullscreen} = usePlayerActions()
    const ref = React.useRef()
    const music = playlist[itemIndex];
    ...
    if(!playlist.length ) return <Box ref={ref}></Box>
    ...
    return (
        <Box ref={ref}
           ...
           <Box sx={{display:"flex", width:"100%", justifyContent:"flex-end", color:"neutral.1"}}>
               <Box
            ...
            <Box sx={{display:"flex",flexDirection:"column" , width:"100%", "& > * ~ *":{
                ...
                <Box sx={{display:"flex", width:"auto"}}>
                    <Box sx={{width:"95px", height:"95px", borderRadius:"5px", overflow:"hidden", "&>img":{height:"100%", width:"auto"}}}><img  src={`http://localhost:4000${music.cover}`} /></Box>
                    <Box sx={{display:"flex", flexDirection:"column", justifyContent:"flex-end", marginLeft:"20px"}}>
                        <Typography variant="h2">{music.title}</Typography>
                        <Typography variant="body3" color="neutral.1">{music.description}</Typography>
                    </Box>
                </Box>
